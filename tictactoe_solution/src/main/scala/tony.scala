...
private final case class MapBoard(moves: List[(Position, Player)], m: M[Position, Player]) extends Board
...
object Main {
  def main(args: Array[String]) {
    ...
    val move = (pos: Position, board: Board, then: Board => Unit) => board --> pos fold (
      { println("taken") },
      { bnext => printBoard[Board](v => v, bnext); then(bnext) },
      { bend => printBoard[FinishedBoard](v => v, bend); println("end") }
    )
    val b1 = EmptyBoard.empty --> NW
